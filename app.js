...
accessPolicyStyles.textContent = `
  ...
  body.admin-readonly [data-admin-view="voters"],
  body.admin-readonly #adminViewVoters,
  body.admin-readonly #adminViewResults .participation-card,
...
function applyAccessLevel(level=currentAccessLevel()){
  ...
  document.querySelector('[data-admin-view="voters"]')?.classList.toggle('readonly-hidden',readonly);
...
window.fetch=async(input,init={})=>{
  ...
  const isMemberLocationsCall=url.includes('/functions/v1/vote-member-locations');
  ...
  const isVoterCodesCall=url.includes('/functions/v1/vote-voter-codes');
  ...
  if(isReadonlyAccess()&&location.hash==='#admin'){
    ...
    if(isMembersCall||isMemberLocationsCall||isBrandingCall||isResultsAdminCall||isCodeAdminCall||isVoterCodesCall){showReadonlyDenied();return accessDeniedResponse('Esta informação é exclusiva dos Administradores Absolutos.');}
