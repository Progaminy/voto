...
if (!sessionStorage.getItem(ADMIN_SESSION_KEY)) sessionStorage.removeItem(ADMIN_ACCESS_KEY);
...
accessPolicyStyles.textContent = `
  ...
  body.admin-readonly #voterCodeCard,
  ...
  @media print {
    ...
    body.admin-readonly::after { content:'Impressão não autorizada para este acesso.';display:block;padding:40px;font:700 18px/1.4 Arial,sans-serif;color:#111; }
    body.admin-direct-print-blocked #adminApp,body.admin-direct-print-blocked #publicApp,body.admin-direct-print-blocked .site-header,body.admin-direct-print-blocked .site-footer { display:none !important; }
    body.admin-direct-print-blocked::after { content:'No painel administrativo, use apenas os botões “Imprimir lista de membros” ou “Imprimir resultados”.';display:block;padding:40px;font:700 18px/1.4 Arial,sans-serif;color:#111; }
  }
`;
...
function applyAccessLevel(level=currentAccessLevel()){
  if(!document.body)return;document.body.classList.toggle('admin-readonly',level==='readonly');document.body.classList.toggle('admin-full',level==='full');const readonly=level==='readonly';
  ...
  document.querySelector('[data-admin-view="settings"]')?.classList.toggle('readonly-hidden',readonly);document.getElementById('adminViewSettings')?.classList.toggle('readonly-hidden',readonly);document.getElementById('toggleElectionBtn')?.classList.toggle('readonly-hidden',readonly);
  document.querySelectorAll('.delete-candidate, .delete-voter, .position-edit-btn, .member-edit-btn, .member-delete-btn, .member-view-code-btn, [data-member-view-code], #printMembersBtn, #printResultsBtn, #adminAccessCodesCard, #voterCodeCard, #announceResultsBtn, #resultRuleCard').forEach(el=>el.classList.toggle('readonly-hidden',readonly));
}
...
window.fetch=async(input,init={})=>{
  const url=typeof input==='string'?input:input?.url||'';const action=getRequestAction(init);const isAdminCall=url.includes('/functions/v1/vote-admin');const isMembersCall=url.includes('/functions/v1/vote-members');const isBrandingCall=url.includes('/functions/v1/vote-branding');const isResultsAdminCall=url.includes('/functions/v1/vote-results-admin');const isCodeAdminCall=url.includes('/functions/v1/vote-view-code-admin');const isVoterCodesCall=url.includes('/functions/v1/vote-voter-codes');const isRestCall=url.includes('/rest/v1/');const isStorageCall=url.includes('/storage/v1/');
  if(isAdminCall&&action==='login'){sessionStorage.removeItem(ADMIN_ACCESS_KEY);applyAccessLevel('');}
  if(isReadonlyAccess()&&location.hash==='#admin'){
    if(isAdminCall&&!READONLY_VOTE_ADMIN_ACTIONS.has(action)){showReadonlyDenied();return accessDeniedResponse();}
    if(isMembersCall&&!READONLY_MEMBER_ACTIONS.has(action)){showReadonlyDenied();return accessDeniedResponse();}
    if(isBrandingCall||isResultsAdminCall||isCodeAdminCall||isVoterCodesCall){showReadonlyDenied();return accessDeniedResponse();}
    const method=String(init.method||(input instanceof Request?input.method:'GET')||'GET').toUpperCase();if((isRestCall||isStorageCall)&&!['GET','HEAD'].includes(method)){showReadonlyDenied();return accessDeniedResponse();}
  }
  let requestInit=init;if(isAdminCall){const headers=new Headers(init.headers||(input instanceof Request?input.headers:undefined));headers.set('apikey',SUPABASE_PUBLIC_KEY);headers.set('x-client-info','axinene-voto/stable-20260902-voter-codes');requestInit={...init,headers};}
...
window.addEventListener('beforeprint',()=>{if(location.hash==='#admin')document.body.classList.add('admin-direct-print-blocked');});window.addEventListener('afterprint',()=>document.body.classList.remove('admin-direct-print-blocked'));
...
async function loadMemberManagement(){if(memberManagementLoaded||location.hash!=='#admin')return;memberManagementLoaded=true;const NativeMutationObserver=window.MutationObserver;class StableMutationObserver extends NativeMutationObserver{observe(target,options={}){if(target===document.body)return;return super.observe(target,options);}}window.MutationObserver=StableMutationObserver;try{await import('./member-management.js?v=20260901-2900');}finally{window.MutationObserver=NativeMutationObserver;}await import('./member-coordination-labels.js?v=20260901-2600');applyAccessLevel();}
window.addEventListener('hashchange',()=>{loadAdminExtras();applyAccessLevel();});
document.addEventListener('click',event=>{if(event.target.closest?.('[data-admin-view="voters"]'))setTimeout(()=>loadMemberManagement(),0);},true);
